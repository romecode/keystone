...
function createItems (data, ops, callback) {
	...
	var nestedLists = {data:{},fields:{},lists:[]};
		...
		async.eachSeries(lists, function (key, doneList) {
			...
			async.eachSeries(data[key], function (data, doneItem) {
				...
				_.keys(data).forEach(function (i) {
					if (typeof data[i] === 'function' && relationshipPaths.indexOf(i) === -1 && listPaths.indexOf(i) === -1) {
						data[i] = data[i]();
						if (options.verbose) {
				...
				async.each(list.fieldsArray, function (field, doneField) {
					...
					if (field.type !== 'relationship' && field.type !== "list") {
						// TODO: Validate items?
	...
	//secondary run for realtionships
	function processRelationships(next,type){
		...
		async.each(lists, function (key, doneList) {
			...
			var fields = _.filter(list.fields, { type: type });
			...
			async.each(data[key], function (srcData, doneItem) {
				...
				if(type=="list" ){
					...
					//for every field in the list model
					async.each(fields, function(listField,_doneList){
						
						//extract field related data
						
						_srcData=listField.getValueFromData(srcData);
						
						if(_srcData){
							var fields = _.filter(listField.fields, { type: 'relationship' });
							
							async.each(_srcData,function(__srcData,dataDone){
								
								forEachRelationshipField(key,list,fields,doc,__srcData,dataDone,doc,relationshipsUpdated,type,listField.path);
							},_doneList)
				...
				}else if (type=="relationship") forEachRelationshipField(key,list,fields,doc,srcData,doneItem,doc,relationshipsUpdated);
	...
	function processLists(next){
		
		processRelationships(null,'list');
		next();
	...
	function forEachRelationshipField(key,list,fields,doc,srcData,doneItem,doc,relationshipsUpdated,type,path){
		
		async.each(fields, function (field, doneField) {
			...
			if (typeof fieldValue === 'function') {
				...
				if (field.many) {
					...
					relationshipsUpdated++;
					//var path = (_nestedLists) ? key : field.path;
					
					if(type=='list' && refsArr.length>0){
						
						var data = {};
						data[field.path]=refsArr;
						_.get(doc,path).addToSet(data);
