/**
 * The form that's visible when "Duplicate <ItemName>" is clicked on the Item screen
 */
...
const DuplicateForm = React.createClass({
	displayName: 'DuplicateForm',
	propTypes: {
	...
	getInitialState () {
		...
		Object.keys(this.props.list.fields).forEach(key => {
			...
			values[field.path] = this.props.data.fields[key];
	...
	renderForm () {
		...
		Object.keys(list.fields).forEach(key => {
			...
			if(list.nameIsInitial && key == nameField.path){
				//do nothing
			}
			else{
				var field = list.fields[key];
				console.log(field)
				if(field.noDuplicate || field.path == "sortOrder"){
					return;
				}
		...
		return (
			<Form layout="horizontal" onSubmit={this.submitForm}>
				<Modal.Header
					text={'Duplicate a ' + list.singular}
					showCloseButton
				/>
				<Modal.Body>
